...
const port = process.env.PORT || parseInt(process.argv.pop()) || 3006;

server.listen(port, function () {
  console.log("Server listening at port %d", port);
});

const RamenOrder = require("./assignment2Ramen");
...
app.post("/payment", (req, res) => {
  ...
  oOrders[sFrom] = new RamenOrder(sFrom);
...
app.post("/sms", (req, res) => {
  ...
  if (!oOrders.hasOwnProperty(sFrom)) {
    oOrders[sFrom] = new RamenOrder(sFrom, sUrl);
  }
  if (oOrders[sFrom].isDone()) {
    delete oOrders[sFrom];
  }
